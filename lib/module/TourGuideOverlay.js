...
import { validateRef, computeTooltipPosition, extractBorderRadius } from "./utils.js";
import { announceStep } from "./accessibility.js";
 ...
 */
const measureElement = (ref, callback) => {
  if (isWeb) {
    try {
      // react-native-web: findNodeHandle returns the DOM node directly
      const node = findNodeHandle(ref.current);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const domNode = node ?? ref.current;
      if (domNode && typeof domNode.getBoundingClientRect === 'function') {
        const rect = domNode.getBoundingClientRect();
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const win = globalThis;
        callback(rect.left + (win.scrollX ?? 0), rect.top + (win.scrollY ?? 0), rect.width, rect.height);
        return;
      }
    } catch {
...
const TourGuideOverlay = ({
  ...
  const getScrollOffset = useCallback(() => {
    return currentStepData?.scrollToTarget?.getCurrentScrollOffset?.() ?? config?.getCurrentScrollOffset?.() ?? 0;
  }, [currentStepData, config]);
  ...
  const measureTarget = useCallback((retryCount = 0) => {
    ...
    measureElement(currentStepData.targetRef, (x, y, width, height) => {
      if (width > 0 && height > 0) {
        const statusBarHeight = !isWeb && Platform.OS === 'android' ? StatusBar.currentHeight ?? 0 : 0;
        ...
        console.warn(`react-native-tour-guide: Step "${currentStepData.id}" target measured as 0x0 after ${MAX_MEASURE_RETRIES} retries.`);
        measureRetryCount.current = 0;
      }
    });
  ...
  const scrollAndMeasure = useCallback(() => {
    ...
    measureElement(currentStepData.targetRef, (_, y, width, height) => {
      if (width <= 0 || height <= 0) {
        measureTarget();
        return;
      }
      ...
      const tooltipSpace = ESTIMATED_TOOLTIP_HEIGHT + (config?.tooltipOffset ?? 8) + (config?.triangleSize ?? 12);
  ...
  }, [currentStepData, measureTarget, getScrollRef, getScrollOffset, screenDimensions.height, config]);
  ...
  useEffect(() => {
    ...
    const measureAll = (retry = 0) => {
      ...
      const finish = () => {
        ...
        setExtraLayouts(results.filter(Boolean));
      };
      refs.forEach((ref, i) => {
        const canMeasure = ref?.current && (isWeb || typeof ref.current.measureInWindow === 'function');
        if (!canMeasure) {
          pending -= 1;
          if (pending === 0) finish();
          return;
        }
        measureElement(ref, (x, y, width, height) => {
          if (width > 0 && height > 0) {
            const statusBarHeight = !isWeb && Platform.OS === 'android' ? StatusBar.currentHeight ?? 0 : 0;
            results[i] = {
              x,
  ...
  // --- Backdrop behavior ---
  const resolveBackdropBehavior = behavior => {
      ...
      if (targetLayout) {
        ...
        if (preferred !== 'auto') {
          ...
          const fits = (() => {
            switch (preferred) {
              case 'bottom':
                return screenDimensions.height - (targetLayout.y + targetLayout.height + offset) >= tooltipHeight;
              case 'top':
                return targetLayout.y - offset >= tooltipHeight;
              case 'left':
                return targetLayout.x - offset >= (config?.tooltipWidth ?? 320) * 0.6;
              case 'right':
                return screenDimensions.width - (targetLayout.x + targetLayout.width + offset) >= (config?.tooltipWidth ?? 320) * 0.6;
              default:
                return false;
            }
          })();
  ...
  const effectiveBorderRadius = currentStepData.spotlightBorderRadius ?? extractBorderRadius(currentStepData.targetStyle);
  ...
  if (config?.renderTooltip && targetLayout) {
    return /*#__PURE__*/_jsxs(AnimatedView, {
      ...
      children: [/*#__PURE__*/_jsx(SpotlightOverlay, {
        ...spotlightProps
  ...
  return /*#__PURE__*/_jsxs(AnimatedView, {
    ...
    children: [/*#__PURE__*/_jsx(SpotlightOverlay, {
      ...spotlightProps
    }), targetLayout ? /*#__PURE__*/_jsx(Tooltip, {
      ...tooltipProps
    }) : null]
  });
};
export default TourGuideOverlay;
//# sourceMappingURL=TourGuideOverlay.js.map